...
export class MudrexExchange implements Exchange {
  ...
  async getRecentFills(c: ApiCredentials, limit = 50): Promise<FillEvent[]> { return list(await this.request(c, 'GET', `/orders?limit=${limit}`)).filter(o => ['FILLED', 'PARTIALLY_FILLED', 'COMPLETED'].includes(String(o.status).toUpperCase())).map(o => ({ externalId: String(o.trade_id ?? o.order_id ?? o.id), symbol: o.symbol, nativeSymbol: o.symbol, canonicalSymbol: o.symbol, quoteCurrency: c.tradeCurrency ?? 'USDT', side: String(o.side).toUpperCase() === 'SELL' ? 'SELL' : 'BUY', qty: n(o.filled_quantity ?? o.executed_quantity ?? o.quantity), price: n(o.average_price ?? o.price), nativePrice: n(o.average_price ?? o.price), reduceOnly: Boolean(o.reduce_only), timestamp: new Date(o.updated_at ?? o.created_at ?? Date.now()) })); }
  async getInstrument(symbol: string): Promise<InstrumentInfo | null> { const response = await fetch(`${this.rest}/assets`); if (!response.ok) return null; const found = list(await response.json()).find(a => a.symbol === symbol); return found ? { symbol, canonicalSymbol: symbol, baseAsset: found.base_asset ?? symbol.replace(/(USDT|INR)$/,''), quoteCurrency: found.quote_asset ?? 'USDT', contractMultiplier: n(found.contract_multiplier) || 1, minQty: n(found.minimum_quantity), qtyStep: n(found.quantity_step) || 1, minNotional: n(found.minimum_notional), orderTypes: ['MARKET'] } : null; }
