  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-[--color-muted]">Verified traders you can mirror. Stats update as they trade.</p>
      </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {leaders.map((l) => (
            <Link key={l.id} href={`/leaders/${l.id}`}>
              <Card className="h-full transition hover:border-[--color-brand]">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="grid h-11 w-11 place-items-center rounded-full bg-[--color-surface-2] text-lg font-semibold">
                      {l.displayName.slice(0, 1)}
                    </div>
                    <div>
                      <div className="font-semibold">{l.displayName}</div>
                      <div className="text-xs text-[--color-faint]">Delta India</div>
                    </div>
                  </div>
                ...
                <div className="mt-5 grid grid-cols-3 gap-2 text-sm">
                  ...
                  <div>
                    <div className="text-xs text-[--color-muted]">Trades</div>
                    <div className="font-semibold tabular-nums">{l.stats.tradeCount}</div>
                  </div>
                  <div>
                    <div className="text-xs text-[--color-muted]">Copied</div>
                    <div className="font-semibold tabular-nums">{fmtUsd(l.stats.totalCopiedUsd, 0)}</div>
                  </div>
                </div>
                <div className="mt-4 flex items-center justify-between border-t border-[--color-border-soft] pt-3">
                  <span className="text-xs text-[--color-muted]">
                    ROI {l.stats.roiPct === 0 ? '—' : fmtPct(l.stats.roiPct)}
                  </span>
                  <span className="text-sm font-medium text-[--color-brand]">Follow →</span>
                </div>
              </Card>
            </Link>
          ))}
